...
--
-- Host: localhost:3306
-- Generation Time: Nov 22, 2022 at 06:59 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `child_records_create` (IN `production_id` VARCHAR(90))  BEGIN
    ...
    SELECT 	tool_changeover_id,no_of_part,count(tool_changeover_id) INTO tool_cid ,loop_end,pmn  FROM pdm_tool_changeover WHERE machine_id = machinename ORDER BY last_updated_on DESC LIMIT 1;
    ...
    IF pmn > 0 THEN
    	...
    	IF i = 1 THEN
        	#updation query
        	UPDATE `pdm_production_info` SET `part_id`=part,`tool_id`=t_id,`production`=production_count,`correction_min_counts`=CONCAT('-',production_count),`rejection_max_counts`=production_count WHERE production_event_id = production_id;
        ELSEIF i > 1 THEN
        	SELECT production_event_id_generation() INTO new_pid;
        	#child record insertion
            INSERT INTO `pdm_production_info`(`production_event_id`,`machine_id`, `calendar_date`, `shift_date`, `shift_id`, `start_time`, `end_time`, `part_id`, `tool_id`, `actual_shot_count`, `production`, `correction_min_counts`,`corrections`, `rejection_max_counts`,`rejections`,`hierarchy`) VALUES(CONCAT('PE',new_pid),machinename,cal_date,sh_date,sh_id,s_time,e_time,part,t_id,actual_scount,production_count,CONCAT('-',production_count),'0',production_count,'0',production_id); 
        END IF;
    END FOR;
	END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` FUNCTION `production_event_id_generation` () RETURNS VARCHAR(90) CHARSET utf8mb4 BEGIN
	...
	SELECT count(r_no) into pid FROM pdm_production_info ORDER BY last_updated_on DESC LIMIT 1;
    IF pid = 0 THEN
    	SET pid = pid+1001;
        RETURN pid;
    ELSE 
...
(1, 99, 80, 65, '2022-11-22 06:53:09', 'UM1001');
...
('0', 'Unplanned', 'Unnamed', '', 1, 1, 'UM1001', '2022-08-17 13:02:25'),
('1', 'Planned', 'Machine OFF', '', 1, 1, 'UM1001', '2022-09-27 10:45:10'),
('2', 'Planned', 'Tool Changeover', '', 1, 1, 'UM1001', '2022-09-27 10:44:37'),
('3', 'Unplanned', 'Tool Changeover', '', 1, 1, 'UM1001', '2022-09-27 10:44:39');
...
(99, 99, 99, 99, 99, 99, 99, 'UM1001', '2022-11-22 06:46:26');
...
('PT1001', 'No Part', 0, 0, 0, 0, 'TL1001', 0, '', 0, 'UM1001', '2022-11-22 06:42:58');
...
--
-- Dumping data for table `settings_part_log`
--

INSERT INTO `settings_part_log` (`part_id`, `part_name`, `NICT`, `part_produced_cycle`, `part_price`, `part_weight`, `tool_id`, `material_price`, `material_name`, `status`, `last_updated_on`, `last_updated_by`) VALUES
('PT1001', 'No Part', 0, 0, 0, 0, 'TL1001', 0, '', '0', '2022-11-22 06:54:54', 'UM1001');
...
('sf01', '09:00:00', '12:00', 'UM1001', '2022-11-22 06:51:12');
